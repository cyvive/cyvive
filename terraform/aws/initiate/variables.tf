...
variable "controller_type" {
  type        = "string"
  default     = "c5.large"
  description = "EC2 instance type for controllers, minimum 'large' class required"
}

variable "pool_type" {
  type        = "string"
  default     = "c5.large"
  description = "EC2 instance type for pools, minimum 'large' class required"
}
...
variable "authorized_external_cidrs" {
	type				= "list"
	default			= ["0.0.0.0/0"]
	description = "CIDR blocks that should be permitted for generic access, its expected that the corporate network would be provided, but defaults to anywhere for public clusters"
}
...
variable "pool_token" {
	description = "Persistent machine token used for nodes in ASG's to auto register with cluster. Must be of the form '[a-z0-9]{6}.[a-z0-9]{16}', autogenerated if not supplied"
	type				= "string"
	default			= ""
}
